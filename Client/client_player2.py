#!/usr/bin/env python3
"""
Pynq-Z2 Pong Game - PLAYER 2 (Blauw Paddel)
UDP Client voor Speler 2
"""
...
PYNQ_PORT_P2 = 5002          # Port voor Speler 2
...
def send_paddle_position(y_position):
    ...
    try:
        ...
        sock.sendto(message, (PYNQ_HOST, PYNQ_PORT_P2))
        sock.close()
        return True
    except Exception as e:
        print(f"[ERROR] Kan niet connecteren naar {PYNQ_HOST}:{PYNQ_PORT_P2}")
        print(f"[ERROR] {e}")
        return False

def main():
    print("\n" + "="*50)
    print("  Pynq-Z2 Pong Game - PLAYER 2 (Blauw)")
    print("="*50)
    print(f"Verbinding met Pynq-Z2: {PYNQ_HOST}:{PYNQ_PORT_P2}")
